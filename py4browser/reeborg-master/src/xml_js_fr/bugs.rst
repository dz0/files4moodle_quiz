...
Picture adapted from the public archive of the `U.S. Naval Historical Center <http://www.history.navy.mil/photos/pers-us/uspers-h/g-hoppr.htm>`__
...
#. Vous pouvez *pauser* un programme en voie d'exécution simplement en
   cliquant sur le bouton **pause** {pause\_button}. Ceci est semblable
...
#. Vous pouvez avoir certaines informations apparaître automatiquement
   quant au nombres de jetons (objet qui sera présenté bientôt) qui sont
   transportés par quelques robots (oui, il est possible d'en avoir
   plusieurs ...), ainsi que la position de ces robots. Pour faire ceci,
   ...
   n'importe où dans un programme et ceci sera considéré valide pour
   tout le programme.

#. Finalement, vous pouvez arrêter complètement l'exécution d'un
   programme en cliquant le bouton **stop** {stop\_button};
